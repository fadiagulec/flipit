...
exports.handler = async function (event) {
    ...
    const userPrompt = [
        ...
        '',
        'Prompt 1 (Main Scene): The core hook moment of the script as a 2-3 scene sequence. Vertical 9:16, anamorphic feel, 24fps, professional color grading. Must include the SCENES block (with timestamps), the VOICEOVER block (slow/measured pacing, ~140 wpm max), and the CAPTIONS block (exact on-screen text per scene with font/placement/animation).',
        '',
        'Prompt 2 (B-Roll Sequence): 3-4 supporting detail shots, each as its own scene with timestamp. Slow motion 60fps, beautiful bokeh, each scene 2-3 seconds. Include VOICEOVER (calm, intimate, unhurried) and CAPTIONS (short keyword overlays per scene).',
        '',
        'Prompt 3 (Transition Sequence): A pivot/contrast moment broken into BEFORE scene → TRANSITION scene → AFTER scene. Cinematic. Include VOICEOVER (measured, never rushed) and CAPTIONS (one phrase per scene, animated entry).',
        '',
        'Make every prompt SPECIFIC to what this script is LITERALLY about — read the words and depict those exact objects/people/places. Reference real moments from the script. No generic lifestyle phrases. NO money screenshots, DM threads, or income notifications unless the script literally talks about those. Every prompt MUST contain explicit SCENES, VOICEOVER, and CAPTIONS sections — do not omit any of these.',
        '',
